...
class RekapPeriodeKasController extends Controller
{
    ...
    public function exportPeriode(Request $request)
    {
        ...
        DB::statement("CREATE OR REPLACE VIEW v_report_rekapkas_bebas AS
                        select ltrim(to_char(rk.rekap,'000')) No_Rekap, to_char(rk.tglrekap,'dd/mm/yyyy') Tanggal_Rekap, rk.jk Jenis_Kartu, (rk.store||' ('||s.namabank||')') Lokasi_Kas_Bank, s.norekening No_Rekening, (d.ci||' ('||mu.namamu||')') Mata_Uang, l.docno as No_Dokumen, d.voucher as No_Bukti, l.keterangan as uraian_penjelasan, l.LOKASI, L.cj, CASE WHEN sign(l.totprice) = '-1' then abs(l.totprice) else '0' end Debet, CASE WHEN -sign(l.totprice) = 1 then 0 else l.totprice end Kredit, $v_saw Saldo_Awal, $v_sak Saldo_Akir from rekapkas rk, kasdoc d, kasline l, storejk s, matauang mu where to_char( rk.tglrekap,'dd-mm-yyyy')=to_char( d.PAIDdate,'dd-mm-yyyy') and rk.store=s.kodestore and rk.store=d.store and rk.jk=d.jk and d.docno=l.docno and coalesce(l.penutup,'N')='N' and d.ci=mu.kodemu and coalesce(d.paid,'N')='Y' And rk.tglrekap Between '$request->tanggal' AND '$request->tanggal2' and rk.jk='$request->jk' and rk.store='$request->nokas' order by d.voucher, l.lineno;
                        ");
        $string_date_one = date('d-m-Y', strtotime($request->tanggal));
        $string_date_two = date('d-m-Y', strtotime($request->tanggal2));
