...
use App\Http\Requests\JurnalUmumStoreRequest;
use App\Http\Requests\JurnalUmumUpdateRequest;
...
class JurnalUmumController extends Controller
{
    ...
    public function store(JurnalUmumStoreRequest $request)
    {
        $validated = collect($request->validated())
            ->put('thnbln', $request->tahun . $request->bulan)
            ->put('posted', 'N')
            ->forget([
                'mp',
                'nomor',
        ...
        JurumDoc::insert($validated);

        Alert::success('Berhasil', 'Data Berhasil Disimpan')->persistent(true)->autoClose(3000);
        return redirect()->route('modul_kontroler.jurnal_umum.edit', [str_replace('/', '-', $request->docno)]);
    }
    ...
    public function edit($no)
    {
        ...
        $data_jur =  DB::select("SELECT docno, left(docno,1) mp, substr(docno, 3, 5) bagian, substr(docno,9) nomor, 
                                    ...
                                    from jurumdoc where docno='$docno'")[0];
    ...
    public function update(JurnalUmumUpdateRequest $request)
    {
        $validated = collect($request->validated())
            ->put('thnbln', $request->tahun . $request->bulan)
            ->forget(['tahun', 'bulan'])
        ...
        if (stbbuku2($validated['thnbln'], $validated['suplesi']) <> 'gtopening') {
            Alert::info('Info', 'Duplikasi data dokumen detail, entri dibatalkan.')->persistent(true)->autoClose(3000);
            return redirect()->route('modul_kontroler.jurnal_umum.index');
        }

        $jurnalUmum = Jurumdoc::where('docno', $validated['docno']);

        if ($jurnalUmum->first()->posted == 'Y') {
            Alert::info('Info', 'Duplikasi data dokumen detail, entri dibatalkan.')->persistent(true)->autoClose(3000);
            return redirect()->route('modul_kontroler.jurnal_umum.index');
        }
        ...
        Alert::info('Berhasil', 'Data Detail Berhasil Diedit.')->persistent(true)->autoClose(3000);
        return redirect()->route('modul_kontroler.jurnal_umum.index');
    }
