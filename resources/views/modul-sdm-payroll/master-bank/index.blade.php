		...
		$('#editRow').click(function(e) {
			...
			if($('input[type=radio]').is(':checked')) { 
				$("input[type=radio]:checked").each(function(){
					...
					location.replace("{{ url('sdm/master_bank/edit') }}"+ '/' +kode);
				});
			} else {
				swalAlertInit('ubah');
			}
				...
				$("input[type=radio]:checked").each(function() {
					...
					const swalWithBootstrapButtons = Swal.mixin({
						...
						swalWithBootstrapButtons.fire({
							...
							text: "Dedail data Master Bank Kode: "+kode,
							type: 'warning',
						...
						if (result.value) {
							$.ajax({
								url: "{{ route('modul_sdm_payroll.master_bank.delete') }}",
								type: 'DELETE',
								dataType: 'json',
								data: {
									"kode": kode,
								...
								success: function () {
									Swal.fire({
										type  : 'success',
										title : "Dedail data Master Bank Kode: "+kode,
										text  : 'Berhasil',
										timer : 2000
									}).then(function() {
										location.reload();
