...
<div class="card card-custom" id="kt_page_sticky_card">
    <div class="card-header">
        <div class="card-title">
            ...
            <h3 class="card-label">
                Menu Edit Pembayaran Permintaan Bayar
            </h3>
        </div>
    </div>
			...
			<span id="form_result"></span>
                <form class="form" id="form-create-detail"  enctype="multipart/form-data">
					...
					<div class="form-group row">
						...
						<label for="example-text-input" class=" col-form-label">:</label>
						<div class="col-8" >
							<select name="status"  class="form-control select2" style="width: 100%;" required oninvalid="this.setCustomValidity('Rincian Harus Diisi..')">
								<option value="">-Pilih-</option>
			...
			<span id="form_result"></span>
                <form class="form" id="form-edit-detail"  enctype="multipart/form-data">
					...
					<div class="form-group row">
						...
						<label for="example-text-input" class=" col-form-label">:</label>
						<div class="col-8">
							<input  class="form-control" type="text" value="" name="nilai" id="nilai1" size="16" maxlength="16"  required oninvalid="this.setCustomValidity('Jumlah Harus Diisi..')" autocomplete="off">
						</div>
					</div>
...
$('#btn-create').on('click', function(e) {
	e.preventDefault();
	$('#title-detail').html("Tambah Detail Pembayaran Permintaan Bayar");
	$('.modal-create').modal('show');
});
$('#btn-delete-all').on('click', function(e) {
	e.preventDefault();
	$('#title-delete-detail').html("Hapus Detail Pembayaran Permintaan Bayar");
...
if($('input[type=radio]').is(':checked')) {  
	$("input[type=radio]:checked").each(function() {  
		...
		var nourut = $(this).attr('nourut');
			$.ajax({
				url :"{{url('perbendaharaan/pembayaran-pbayar/editdetail')}}"+ '/' +nodok+ '/' +nourut,
				type : 'get',
				dataType:"json",
				headers: {
					'X-CSRF-Token': '{{ csrf_token() }}',
				...
				success:function(data)
				{
					...
					$('#nilai1').val(rupiah);
					$('#title-edit-detail').html("Edit Detail Pembayaran Permintaan Bayar");
			...
			if($('input[type=radio]').is(':checked')) { 
				$("input[type=radio]:checked").each(function() {
					...
					.then((result) => {
						if (result.value) {
							$.ajax({
								url: "{{ route('pembayaran_pbayar.delete.detail') }}",
								type: 'DELETE',
								dataType: 'json',
								data: {
									"nodok": nodok,
