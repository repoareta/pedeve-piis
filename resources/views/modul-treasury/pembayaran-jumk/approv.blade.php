...
<div class="card card-custom card-sticky" id="kt_page_sticky_card">
    <div class="card-header">
        <div class="card-title">
            ...
            <h3 class="card-label">
                @if($status == 'Y')
				Tabel <span style="color:blue;">Pembatalan</span> Approval Pemabayaran Pertanggungjawaban UMK Pekerja
                @elseif($status == 'N')
				Tabel <span style="color:blue;">Eksekusi</span> Approval Pemabayaran Pertanggungjawaban UMK Pekerja
                @endif
            </h3>
        </div>
    </div>
    ...
    <div class="card-body">
        <form class="form" action="{{ route('pembayaran_jumk.store.app') }}" method="POST">
            @csrf
            <div class="form-group row">
                <label for="mulai-input" class="col-2 col-form-label">No.Dokumen</label>
                <div class="col-10">
                    <input style="background-color:#e4e6ef; cursor:not-allowed" type="text" class="form-control" name="nodok" value="{{ $data->docno }}" readonly />
